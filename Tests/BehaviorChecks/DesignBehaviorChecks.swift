...
struct DesignBehaviorChecks {
    ...
    private static func checkFavoritesListsUseMusicHoverAndSongTable() throws {
        ...
        precondition(libraryView.contains("private struct FavoriteSongsDoubleClickMonitor: NSViewRepresentable"))
        precondition(libraryView.contains("FavoriteSongsDoubleClickMonitor("))
        precondition(libraryView.contains("NSEvent.addLocalMonitorForEvents(matching: .leftMouseDown)"))
        precondition(libraryView.contains("event.clickCount >= 2"))
    ...
    private static func checkHistoryRefreshDoesNotDriveWindowToolbar() throws {
        ...
        precondition(libraryView.contains("playbackEngine: appState.playbackEngine"))
        ...
        precondition(!libraryView.contains("isSelected: selectedTrackID.wrappedValue == entry.id"))
        precondition(libraryView.contains("private struct HistorySongsDoubleClickMonitor: NSViewRepresentable"))
        precondition(libraryView.contains("HistorySongsDoubleClickMonitor("))
        precondition(libraryView.contains("isPlaying: playbackEngine.currentItem?.track.id == entry.id"))
