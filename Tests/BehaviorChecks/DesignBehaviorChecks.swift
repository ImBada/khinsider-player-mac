...
struct DesignBehaviorChecks {
    ...
    private static func checkAlbumTopBarAppearsOnlyAfterScrolling() throws {
        ...
        precondition(!source.contains("albumBackButtonClickProxy"))
        ...
        precondition(source.contains("AlbumTopControls("))
        precondition(source.contains(".zIndex(AlbumDetailLayout.topControlsZIndex)"))
        precondition(source.contains("albumURL: album.url"))
        ...
        precondition(source.contains(".padding(.horizontal, AlbumDetailLayout.topControlDragExclusionWidth)"))
        precondition(source.contains("window?.performDrag(with: event)"))
        precondition(source.contains("override func mouseDown(with event: NSEvent)"))
        precondition(!source.contains("controlExclusionRects"))
        precondition(!source.contains("AlbumBackButtonClickProxy"))
        precondition(!source.contains("ClickProxyView"))
        precondition(!source.contains("topControlClickProxy"))
        precondition(!source.contains("NSEvent.addLocalMonitorForEvents(matching: .leftMouseDown)"))
        precondition(source.contains("static let topBarDragAreaZIndex: Double = 15"))
        precondition(source.contains("static let topControlsZIndex: Double = 30"))
        precondition(source.contains("static let topControlHorizontalPadding: CGFloat = 22"))
        precondition(source.contains("static let topControlDragExclusionWidth: CGFloat = 104"))
    }
